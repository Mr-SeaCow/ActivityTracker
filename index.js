...
console.log(Date.parse(getTimestamp()));
console.log(Date.now());

console.log(Date(getTimestamp()) == Date.now());
...
client.on(Events.MessageCreate, async message => {
  ...
  if (message.content === 'act') {
    ...
    for (const key in Users) {
        ...
        if (lastJoinTime !== 0)
            totalTime += Date.now() - lastJoinTime;
    ...
    for (const key in Users) {
        ...
        console.log(tUser.user.username)
    ...
    str += '```'
    console.log(colLengths.reduce((a, b) => a + b, 0))
    console.log(str)
    // console.table(ara, ['Username', 'Messages', 'Voice Time', 'Activity']);
